      ...
      >
        {dragging && (
          <div className="absolute inset-0 z-10 flex items-center justify-center rounded-2xl border-2 border-dashed border-brand-400 bg-brand-50/80 backdrop-blur-sm animate-fade-in">
            <div className="text-center">
              ...
              <p className="text-sm font-medium text-brand-700">Déposez vos fichiers ici</p>
          ...
          <div>
            ...
            {docs && (
              <p className="mt-1 text-sm text-slate-500">
                {docs.length} document{docs.length !== 1 ? 's' : ''} indexé{docs.length !== 1 ? 's' : ''}
              </p>
            )}
          </div>
        ...
        {docs === null ? (
                  ...
                  <tr key={d.id} className="transition-colors hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => onReindex(d.id)} className="rounded-md p-1.5 text-slate-400 transition-colors hover:bg-slate-100 hover:text-brand-600" title="Réindexer">
                          <RefreshCw size={14} />
                        </button>
                        <button onClick={() => onDelete(d.id)} className="rounded-md p-1.5 text-slate-400 transition-colors hover:bg-red-50 hover:text-red-600" title="Supprimer">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
          ...
          <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-white py-16">
            ...
            <p className="mb-1 text-sm font-medium text-slate-700">Aucun document</p>
            <p className="mb-4 text-xs text-slate-500">Importez des fichiers ou glissez-déposez pour commencer</p>
